    ...
    if('r' in a[0]):
        ...
        if(time == 'simple_present'):
            if(conjugation in ['i', 'you', 'we', 'they']):
                ...
                if any(word.endswith(part) for part in ['s', 'x', 'sh', 'ch']):         # ((a[1][-1] in ['s', 'x']) or (a[1][-2:-1] in ['sh', 'ch'])):
                    return word + 'es'
                elif word.endswith('y') and word[-2] not in ['a', 'e', 'i', 'o', 'u']:  #(a[1][-1] in 'y' and a[1][-2] not in ['a', 'e', 'i', 'o', 'u']):
                    return word[:-1]+'ies'
                else:
                    return word+'s'
        ...
        return a[2][time][conjugation]      # [x for x in a[2] if x[0] is time][0][1][conjugation]
...
def noun(singularity = 's', compound = False, n = noun_list.noun_list):
    ...
    if(compound):
        if(singularity == 's'):
            return random.choice(nb)[1] + ' ' + random.choice([word for type_, word in ne if 's' in type_])
        if(singularity == 'm'):
            x = random.choice(nb)[1] + ' ' + random.choice([word for type_, word in ne if 'm' in type_])
            
            # x is a string (consisting of 2 words) at this point, x[1] is a 2nd letter, x[1][-2:-1] evaluates to empty string
            # I assume it was intended to be the ending 1 or 2 letters of the 2nd word so str.endswith() is used instead (and 2nd word happens to be at the end of the string)

            if any(x.endswith(part) for part in ['s', 'x', 'sh', 'ch']):        #((x[1][-1] in ['s', 'x']) or (x[1][-2:-1] in ['sh', 'ch'])):
                return x + 'es'
            if x[-1] == 'y' and x[-2] not in ['a', 'e', 'i', 'o', 'u']:         # (x[-1] in ['y'] and x[-2] not in ['a', 'e', 'i', 'o', 'u']):
                return x[:-1] + 'ies'
            return x + 's'
    else:
        if(singularity == 's'):
            return random.choice([word for type_, word in nb+ne+na if 's' in type_])
        else:
            x = random.choice([word for type_, word in nb+ne+na if 'm' in type_])

            # again x is a string

            if any(x.endswith(part) for part in ['s', 'x', 'sh', 'ch']):        # ((x[1][-1] in ['s', 'x']) or ((x[1][-2:-1] in ['sh', 'ch']))):
                return x + 'es'
            if x[-1] == 'y' and x[-2] not in ['a', 'e', 'i', 'o', 'u']:
                return x[:-1] + 'ies'
            return x + 's'
